# -*- coding: utf-8 -*-
"""plan2_gan_models.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1dM9Jg4WOjnvkSrBcMaBCKyOoWWK3aqPW
"""
...
class ConditionalGANGenerator(nn.Module):
    """
    Generator for Conditional GAN (cGAN) to produce embeddings conditioned on labels.
    ...
    def __init__(self, latent_dim, embedding_dim, num_classes):
        ...
        self.model = nn.Sequential(
            nn_utils.spectral_norm(nn.Linear(latent_dim + num_classes, 512)),  # Latent + conditional labels as input
    ...
    def forward(self, z, labels):
        ...
        Args:
            ...
            labels (torch.Tensor): Conditional latent vector (one-hot encoded class labels or indices).

        Returns:
            torch.Tensor: Generated embedding conditioned on the latent variables.
        """
        # Ensure labels are one-hot encoded if they are class indices (1D tensor)
        if labels.ndimension() == 1:  # If `labels` is a 1D vector (e.g., class indices), make it one-hot
            labels = F.one_hot(labels, num_classes=self.categorical_dim).float()  # Convert to one-hot
        elif labels.ndimension() == 2 and labels.size(1) != self.categorical_dim:
            raise ValueError(f"Expected labels with size (batch_size, {self.categorical_dim}), but got {labels.size()}.")

        # Concatenate the latent vector `z` and one-hot encoded `labels`
        z = torch.cat([z, labels], dim=1)  # Concatenate along the feature dimension
        return self.model(z)
